...
const QuickTransfer: React.FC<QuickTransferProps> = ({ quickTransfer }) => {
  ...
  const handleSendMoney = () => {
    ...
    toast.success(
      `Sent $${amount} to ${quickTransfer.find((user) => user.id === selectedUser)?.name
      }`
    );
    setAmount(0);
            ...
            {quickTransfer
              .map(({ id, name, role, avatar }) => (
                ...
                >
                  ...
                  <p
                    className={`text-text-primary text-base text-[16px] mt-2 w-fit ${selectedUser === id ? "font-extrabold" : "font-medium"
                      }`}
                  >
                    {name}
                  </p>
                  <p
                    className={`text-secondary text-[15px] w-fit ${selectedUser === id ? "font-extrabold" : "font-normal"
                      }`}
                  >
                    {role}
                  </p>
